...
IF "%IDE_BIN_DIR:~-8%" == "bin\win\" (
  echo.
  echo    You are trying to run SpecEdit from generic distribution
  echo Please first copy content of "%IDE_BIN_DIR%"
  echo into its parent directory ("%IDE_BIN_DIR:~0,-4%"^)
  echo and then run specedit.bat from there
  echo.
  pause
  exit
)
...
:: Try (in order): SpecEdit_JDK, specedit%BITS%.exe.jdk, ..\jre, JDK_HOME, JAVA_HOME.
:: ---------------------------------------------------------------------
SET JDK=

IF EXIST "%SpecEdit_JDK%" SET JDK=%SpecEdit_JDK%
...
IF NOT EXIST "%JAVA_EXE%" (
  ECHO ERROR: cannot start SpecEdit.
  ECHO No JDK found. Please validate either SpecEdit_JDK, JDK_HOME or JAVA_HOME points to valid JDK installation.
  ECHO
  EXIT /B
)
...
:: ---------------------------------------------------------------------
IF NOT "%SpecEdit_PROPERTIES%" == "" SET IDE_PROPERTIES_PROPERTY="-Didea.properties.file=%SpecEdit_PROPERTIES%"

:: explicit
SET VM_OPTIONS_FILE=%SpecEdit_VM_OPTIONS%
...
IF NOT EXIST "%VM_OPTIONS_FILE%" (
  ...
  SET VM_OPTIONS_FILE=%IDE_BIN_DIR%\specedit%BITS%.exe.vmoptions
)
IF NOT EXIST "%VM_OPTIONS_FILE%" (
  :: default, universal package
  SET VM_OPTIONS_FILE=%IDE_BIN_DIR%\win\specedit%BITS%.exe.vmoptions
)
IF NOT EXIST "%VM_OPTIONS_FILE%" (
