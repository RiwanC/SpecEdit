...
# Try (in order): SpecEdit_JDK, specedit.jdk, ../jre, JDK_HOME, JAVA_HOME, "java" in PATH.
# ---------------------------------------------------------------------
...
if [ -z "$JDK" ] || [ ! -x "$JAVA_BIN" ]; then
  message "No JDK found. Please validate either SpecEdit_JDK, JDK_HOME or JAVA_HOME environment variable points to valid JDK installation."
  exit 1
fi
...
# ---------------------------------------------------------------------
if [ -n "$SpecEdit_PROPERTIES" ]; then
  IDE_PROPERTIES_PROPERTY="-Didea.properties.file=$SpecEdit_PROPERTIES"
fi
...
if [ -n "$IDEA_VM_OPTIONS" -a -r "$IDEA_VM_OPTIONS" ]; then
  ...
  VM_OPTIONS_FILE="$IDE_BIN_HOME/$OS_SPECIFIC/specedit$BITS.vmoptions"
fi
...
if [ -n "$SpecEdit_CLASSPATH" ]; then
  CLASSPATH="$CLASSPATH:$SpecEdit_CLASSPATH"
fi
