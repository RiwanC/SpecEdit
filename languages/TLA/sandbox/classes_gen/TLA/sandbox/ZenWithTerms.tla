...
commitRequests ≜ {[method ↦ Commit,source ↦ n,dest ↦ ns,term ↦ currentTerm[n],version ↦ lastPublishedVersion[n]] : ns ∈ Nodes} IN  ⋀ messages '  = messages ∪ commitRequests ELSE  UNCHANGED 〈messages〉 ∧  UNCHANGED 〈startedJoinSinceLastReboot,lastCommittedConfiguration,currentTerm,electionWon,descendant,lastAcceptedVersion,lastAcceptedValue,lastAcceptedTerm,lastAcceptedConfiguration,lastPublishedVersion,lastPublishedConfiguration,joinVotes,initialConfiguration,initialValue,initialAcceptedVersion〉
...
--------------------
SingleNodeInvariant ≜  ∀ n ∈ Nodes :  ⋀ lastAcceptedTerm[n] ≤ currentTerm[n] ∧ electionWon[n] = IsElectionQuorum(n,joinVotes[n]) ∧  IF electionWon[n] THEN lastPublishedVersion[n] ≥ lastAcceptedVersion[n] ELSE lastPublishedVersion[n] = 0 ∧ electionWon[n] ⇒ startedJoinSinceLastReboot[n] ∧ publishVotes[n] ≠ {} ⇒ electionWon[n]
...
CommittedValuesDescendantsFromInitialValue ≜  ∃ v ∈ InitialVersions :  ⋀  ∃ n ∈ Nodes : v = initialAcceptedVersion[n] ∧  ∃ votes ∈  SUBSET (initialConfiguration) :  ⋀ IsQuorum(votes,initialConfiguration) ∧  ∀ n ∈ votes : initialAcceptedVersion[n] ≤ v ∧  ∀ m ∈ messages : CommittedPublishRequest(m) ⇒ [prevT ↦ 0,prevV ↦ v,nextT ↦ m . term,nextV ↦ m . version] ∈ descendant
...
StateConstraint ≜  ⋀  ∀ n ∈ Nodes :  IF currentTerm[n] ≤ 1 THEN lastPublishedVersion[n] ≤ 2 ELSE lastPublishedVersion[n] ≤ 3 ∧ Cardinality(messages) ≤ 15
====================